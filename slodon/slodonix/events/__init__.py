...
class Field(object):
    ...
    def parse_binary_value(self, data, display, length, format):
        ...
        """
        raise RuntimeError(
            "Neither structcode or parse_binary_value " "provided for {0}".format(self)
        )
...
class List(ValueField):
    ...
    def pack_value(self, val):
        ...
        if self.type.structcode and len(self.type.structcode) == 1:
            data = array(struct_to_array_codes[self.type.structcode], val).tobytes()
        else:
            data = []
            for v in val:
...
class Struct:
    ...
    def to_binary(self, *varargs, **keys):
        ...
        for f in self.var_fields:
            ...
            code = code + (
                "  _%(name)s, _%(name)s_length, _%(name)s_format"
        ...
        for f in self.static_fields:
            if isinstance(f, LengthField):
                ...
                if isinstance(f, TotalLengthField):
                    if self.var_fields:
                        pack_args.append(
                            "self.static_fields[%d].calc_length(%s)" % (i, total_length)
                        )
                    else:
                        pack_args.append(str(f.calc_length(self.static_size)))
                else:
                    pack_args.append(
                        "self.static_fields[%d].calc_length(_%s_length)" % (i, f.name)
                    )
            ...
            # Value fields
                ...
                if f.structvalues == 1:
                    ...
                    if f.check_value is not None:
                        pack_args.append(
                            "self.static_fields[%d].check_value(%s)" % (i, f.name)
                        )
        ...
        for f in self.static_fields:
            ...
            # when treating varfields.
                ...
                if f.parse_value is None:
                    ...
                    lengths[
                        f.name
                    ] = "self.static_fields[%d]." "parse_value(val[%d], display)" % (
                        fno,
                        vno,
                    )
...
class FixedPropertyData(PropertyData):
    ...
    def parse_binary_value(self, data, display, length, format):
        return PropertyData.parse_binary_value(
            self, data, display, self.size // (format // 8), format
        )
...
class Event(GetAttrData):
    def __init__(self, binarydata=None, display=None, **keys):
        if binarydata:
            ...
            self._data, data = self._fields.parse_binary(
                binarydata, display, rawdict=True
            )
